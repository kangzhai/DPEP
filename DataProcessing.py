...
def OriginalComplexFeatureConstruction(feature1, feature2):
    fpair = ''
...
def BriefComplexFeatureConstruction(feature1, feature2):
    fpair = ''
    if feature1 != '' and feature2 != '':
        f1, f2 = feature1.strip().split(' '), feature2.strip().split(' ')
        for i in range(len(f1)):
...
def FusionComplexFeature(ListPair):
    OriginalComplexFeatureVector, BriefComplexFeatureVector = [], []
    for LinePair in ListPair:
        ...
        SequencemiRNAGgap = SequenceGgapExtract(miRNAsequence, SequenceGgaptotal)
        ...
        SequencelncRNAGgap = SequenceGgapExtract(lncRNAsequence, SequenceGgaptotal)
        ...
        Osequencekmer = OriginalComplexFeatureConstruction(SequencemiRNAkmer, SequencelncRNAkmer)
        OsequenceGgap = OriginalComplexFeatureConstruction(SequencemiRNAGgap, SequencelncRNAGgap)
        Ostructurekmer = OriginalComplexFeatureConstruction(StructuremiRNAkmer, StructurelncRNAkmer)
        OstructureGgap = OriginalComplexFeatureConstruction(StructuremiRNAGgap, StructurelncRNAGgap)
        # Fusion
        OFeatureLine = Osequencekmer + OsequenceGgap + Ostructurekmer + OstructureGgap
        OFeatureLine = OFeatureLine.strip()
        OriginalComplexFeatureVector.append(OFeatureLine)
        # Brief Complex Feature
        Bsequencekmer = BriefComplexFeatureConstruction(SequencemiRNAkmer, SequencelncRNAkmer)
        BsequenceGgap = BriefComplexFeatureConstruction(SequencemiRNAGgap, SequencelncRNAGgap)
        Bstructurekmer = BriefComplexFeatureConstruction(StructuremiRNAkmer, StructurelncRNAkmer)
        BstructureGgap = BriefComplexFeatureConstruction(StructuremiRNAGgap, StructurelncRNAGgap)
        # Fusion
        BFeatureLine = Bsequencekmer + BsequenceGgap + Bstructurekmer + BstructureGgap
        BFeatureLine = BFeatureLine.strip()
        BriefComplexFeatureVector.append(BFeatureLine)
    return OriginalComplexFeatureVector, BriefComplexFeatureVector

def ConvertToMatrix(ListVector, Frow):
    X_test = []
    Fcolumn = math.ceil((len(ListVector[0].split(' '))) / Frow)
    for line in ListVector:
        Feature = line.split(' ')
...
def ConvertToVector(ListVector):
    rowtestdata = len(ListVector)
    columntestdata = len(ListVector[0].split())
    testfeature = [([0] * columntestdata) for p in range(rowtestdata)]
    for indextestdata in range(rowtestdata):
        linetestdata = ListVector[indextestdata]
        setestdata = linetestdata.strip().split(' ')
        for itestdata in range(len(setestdata)):
